import { Client, Databases, ID } from 'node-appwrite';

export default async ({ req, res, log, error }) => {

  const client = new Client();
  client.setEndpoint("https://cloud.appwrite.io/v1");
  client.setProject("664eb7990012ea2e93ff");
  client.setKey("");

  const databases = new Databases(client);

  const result1 = await databases.createCollection("668da72e003067d8f83f", ID.unique(), "Demo Battle Room");
  const result2 = await databases.createStringAttribute("668da72e003067d8f83f", result1.$id, "player", 20, true);

  log("Test");
  log(result1.$id);

  return res.send(result1.$id);

};
